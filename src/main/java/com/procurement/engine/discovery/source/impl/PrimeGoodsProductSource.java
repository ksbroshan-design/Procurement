...
/**
 * Discovery source for PrimeGoods Distribution (Third Party Marketplace / Suspended Vendor).
 * <p>
 * Demonstrates deterministic handling of unavailable/suspended vendors.
 */
@Component
public class PrimeGoodsProductSource implements ProductDiscoverySource {

    private static final Logger log = LoggerFactory.getLogger(PrimeGoodsProductSource.class);
    public static final String SOURCE_NAME = "PrimeGoods Marketplace";
    public static final String VENDOR_NAME = "PrimeGoods Distribution (Suspended)";

    private final ProductRepository productRepository;
    private final VendorRepository vendorRepository;

    public PrimeGoodsProductSource(ProductRepository productRepository, VendorRepository vendorRepository) {
    ...
    public DiscoverySourceResult discover(String category, int requestedQuantity) {
        ...
        if (vendor.getStatus() != VendorStatus.ACTIVE) {
            log.info("Vendor [{}] status is [{}]. Excluding from valid purchase candidates.", VENDOR_NAME, vendor.getStatus());
            return DiscoverySourceResult.failure(SOURCE_NAME, "Vendor '" + VENDOR_NAME + "' is currently " + vendor.getStatus() + " and cannot fulfill purchase orders.");
        }
